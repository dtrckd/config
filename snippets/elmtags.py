...
for lnum, line in enumerate(file_content):
    ...
    match_function_lazy = re_function_lazy.match(line)
    cur_searchterm = ""
    cur_kind = ""
    args = ""
    lines = ""
    ...
    if match_header:
        cur_head = match_header.group(1).strip()
        cur_tag = cur_head
        ...
        cur_tag = match_function.group(1).strip()
        cur_searchterm = "^" + cur_tag + " :"
        cur_kind = "f"
        args = "\theader:" + cur_head + "\tsignature:(" + match_function.group(2) + ")"
        func_list.append(cur_tag)
    elif match_function_lazy:
        cur_tag = match_function_lazy.group(1).strip()
        if cur_tag in func_list:
            continue
        cur_searchterm = "^" + cur_tag + " :"
        cur_kind = "f"
        args = "\theader:" + cur_head
